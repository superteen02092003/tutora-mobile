...
class _StudentMarketplacePageState extends ConsumerState<StudentMarketplacePage>
  ...
  List<FilterOption> _subjectOptions(WidgetRef ref) {
    return ref
        .watch(subjectsProvider)
        .maybeWhen(
          data: (list) => list
              .map(
                (s) => (key: s.subjectId.toString(), label: s.subjectName),
              )
              .toList(),
  ...
  List<FilterOption> _gradeOptions(WidgetRef ref) {
    return ref
  ...
  Widget build(BuildContext context) {
    ...
    final subjectId = state is MarketplaceLoaded
        ...
        : null;
    final subjectOpts = _subjectOptions(ref);
    final gradeOpts = _gradeOptions(ref);

    return Scaffold(
      backgroundColor: AppColors.cream,
      body: SafeArea(
        // Let the injected nav-height bottom padding pass through to the list.
        bottom: false,
        child: Column(
        ...
        child: Column(
          ...
          children: [
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // Squared avatar in the top-left corner (not full height).
                ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: UserAvatar(
...
// Subject picker — bottom sheet that filters by a single subject.
