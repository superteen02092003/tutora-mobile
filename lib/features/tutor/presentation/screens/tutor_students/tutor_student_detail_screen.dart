...
class TutorStudentDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final students = ref.watch(recorderStudentsProvider).valueOrNull ?? const [];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: () => TutorStudentFormScreen.open(context, student: student),
            child: const Text('Sửa', style: TextStyle(color: TutorColors.primary)),
          ),
        ],
      ),
          ...
          children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(student.fullName,
                          style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w700, color: TutorColors.ink)),
                      const SizedBox(height: 3),
                      Text(
                        student.subtitle.isEmpty ? 'Ngoài nền tảng' : '${student.subtitle} · Ngoài nền tảng',
            ...
            _Card(
              children: [
                _Row(
                  ...
                  title: student.parentName?.isNotEmpty == true ? student.parentName! : 'Chưa có tên phụ huynh',
                ...
                _Row(
                  icon: student.hasConsent ? Icons.verified_outlined : Icons.mic_off_outlined,
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                label: const Text('Ghi âm buổi học', style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600)),
              ),
            ),
            const SizedBox(height: 26),
            ...lessons.when<List<Widget>>(
              loading: () => [const Padding(padding: EdgeInsets.all(20), child: Center(child: CircularProgressIndicator()))],
              error: (_, _) => [const Text('Không tải được danh sách buổi. Kéo xuống để thử lại.')],
              data: (list) {
                final now = DateTime.now();
                // Sắp tới: 3 buổi đã lên lịch gần nhất, tăng dần theo giờ.
                final upcoming = list
                    .where((l) => l.status == 'scheduled' && (l.scheduledEnd ?? l.when ?? now).isAfter(now))
                    .toList()
                  ..sort((a, b) => a.when!.compareTo(b.when!));
                // Đã ghi: mới nhất trước.
                final recorded = list.where((l) => l.status != 'scheduled').toList()
                  ..sort((a, b) => (b.when ?? now).compareTo(a.when ?? now));
                return [
                  if (upcoming.isNotEmpty) ...[
                    const _Section('SẮP TỚI'),
                    for (final l in upcoming.take(3)) ...[
                      _LessonTile(lesson: l, studentName: student.fullName, subtitle: student.subtitle),
                  ...
                  if (recorded.isEmpty)
                    const Text('Chưa có buổi nào. Bấm "Ghi âm buổi học" khi bắt đầu dạy.',
                        style: TextStyle(fontSize: 13, color: TutorColors.ink3))
                  else
                    for (final l in recorded) ...[
                      RecordedLessonTile(lesson: l, studentName: student.fullName),
                      const SizedBox(height: 10),
                    ],
                ];
              },
...
Future<void> startStudentRecording(
  ...
  if (ref.read(lessonRecordingProvider).isRecording) {
    ...
    onBeforeStart?.call();
    if (target != null) unawaited(router.push(AppRoutes.tutorRecording, extra: target));
    return;
  }
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Đã đồng ý')),
        ],
      ),
    );
    if (ok != true || !context.mounted) return;
    try {
      s = await ref.read(recorderDatasourceProvider).updateStudent(
        s.studentId,
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Chưa lưu được đồng ý. Kiểm tra mạng rồi thử lại.')),
        );
      }
      return;
    }
  ...
  onBeforeStart?.call();
  unawaited(router.push(AppRoutes.tutorRecording, extra: RecordingTarget.fromStudent(s)));
}

class _LessonTile extends StatelessWidget {
  const _LessonTile({required this.lesson, required this.studentName, required this.subtitle});
  ...
  Widget build(BuildContext context) {
    ...
    final (label, bg, fg) = switch (lesson.status) {
      ...
      'recording' || 'uploading' => ('Đang ghi', TutorColors.accentBg, TutorColors.warning),
      _ => ('Đã lên lịch', TutorColors.surfaceSunken, TutorColors.ink3),
    };
    final recorded = !{'scheduled', 'recording', 'uploading'}.contains(lesson.status);
    return Material(
      ...
      child: InkWell(
        onTap: recorded
            ? () => TutorRecordingDetailScreen.open(
                  context,
                  RecordingDetailArgs(recordingId: lesson.lessonId, studentName: studentName),
                )
            : null,
        child: Padding(
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(date, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: TutorColors.ink)),
                    if (lesson.durationSec > 0) ...[
                      const SizedBox(height: 3),
                      Text('${(lesson.durationSec / 60).round()} phút',
                          style: const TextStyle(fontSize: 13, color: TutorColors.ink3)),
                    ],
                  ],
                ),
...
class _Row extends StatelessWidget {
  const _Row({required this.icon, required this.title, required this.sub, this.warn = false});
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(
      children: [
        Icon(icon, size: 19, color: warn ? TutorColors.warning : TutorColors.ink3),
        const SizedBox(width: 12),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: TutorColors.ink)),
              const SizedBox(height: 2),
              Text(sub, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: warn ? TutorColors.warning : TutorColors.ink4)),
            ],
          ),
        ),
      ],
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: TextStyle(fontSize: fontSize, fontWeight: FontWeight.w700, color: TutorColors.ink),
      ),
    );
  }
}
...
class _Section extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Text(
      text,
      style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, letterSpacing: 0.8, color: TutorColors.ink4),
    ),
  );
}
